...
API route принимает данные формы заявки, валидирует их через Zod, опционально сохраняет заявку в Google Sheets через Google Apps Script, отправляет Telegram-уведомление владельцу и возвращает результат.
...
Если `GOOGLE_APPS_SCRIPT_WEBHOOK_URL` не задан, `googleSheetsSynced` будет `false`, потому что запись в таблицу пропущена.
