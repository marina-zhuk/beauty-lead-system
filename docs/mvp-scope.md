...
- запись заявки в Google Sheets через Google Apps Script;
- README;
- `.env.local.example`.
...
В текущей реализации `POST /api/lead` принимает данные, валидирует их через Zod, отправляет Telegram-уведомление владельцу, передает заявку в Google Apps Script для записи в Google Sheets и возвращает `success/error`.
